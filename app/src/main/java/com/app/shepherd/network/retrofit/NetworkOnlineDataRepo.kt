    ...
    fun asFlow() = flow {
        ...
        try {
            ...
            if (apiResponse.isSuccessful && data != null) {
                ...
                // emit(com.app.shepherd.network.retrofit.DataResult.Failure("Something went wrong!"))
                /*if (apiResponse.code() == 401) {
                    // logout the user
                    emit(com.app.shepherd.network.retrofit.DataResult.Failure(getErrorMsg(apiResponse.errorBody()!!),false,apiResponse.code()))

                 } else {
                    emit(com.app.shepherd.network.retrofit.DataResult.Failure(getErrorMsg(apiResponse.errorBody()!!),true,apiResponse.code()))
                }*/
